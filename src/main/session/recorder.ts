...
function averageStepTime(steps: Step[]): number {
  const timings = steps.map((s) => s.delayMs).filter((d): d is number => typeof d === 'number' && d > 0)
...
export function saveToNode(graph: LearningGraph, nodeId: string, steps: Step[]): LearningGraph {
  ...
  next.nodes[nodeId] = {
    ...
    avgStepTimeMs:
      currentNode.attempts === 0
        ? avgStepTimeMs
        : (currentNode.avgStepTimeMs * currentNode.attempts + avgStepTimeMs) / (currentNode.attempts + 1),
