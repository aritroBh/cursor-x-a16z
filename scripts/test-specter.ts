...
async function main() {
  ...
  check(whisper.includes('classifyWhisperError'), 'whisper.ts classifies errors for the renderer')
  check(whisper.includes('WhisperResult'), 'whisper.ts returns structured result objects')
  check(whisper.includes('gpt-4o-mini-transcribe') && whisper.includes('whisper-1'), 'whisper.ts supports gpt-4o-mini-transcribe with whisper-1 fallback')
  check(inputBar.includes('result.message || "No transcription returned'), 'InputBar displays specific Whisper error messages')
  ...
  check(whisper.includes('[WHISPER] received buffer'), 'whisper.ts logs buffer reception')
  ...
  check(whisper.includes('[WHISPER] transcription failed completely'), 'whisper.ts logs transcription failures')
  ...
  check(inputBar.includes('setValue(text)') && !inputBar.includes('onSubmit(text)'), 'transcription populates input instead of submitting empty/implicit text')
  ...
  check(overlayApp.includes('Check Voice Backend'), 'Debug UI exposes Check Voice Backend')
  check(overlayApp.includes('Test Voice Output'), 'Debug UI exposes Test Voice Output')
  ...
  check(health.includes('ELEVENLABS_API_KEY') && health.includes('ElevenLabsHealth'), 'AI health reports ElevenLabs configuration')
  check(health.includes('openaiTTS') && health.includes('OpenAITTSHealth'), 'AI health reports OpenAI TTS configuration')
  check(health.includes('readyForNaturalVoiceOutput = result.elevenlabs.configured || result.openaiTTS.configured'), 'AI health marks natural voice ready if either provider is available')
  ...
  check(mainIndex.includes('isBrokenPipeError'), 'index.ts defines isBrokenPipeError')
  check(mainIndex.includes("code === 'EPIPE'") || mainIndex.includes('code === "EPIPE"'), 'isBrokenPipeError checks for EPIPE code')
  check(mainIndex.includes("process.on('uncaughtException'") && mainIndex.includes('isBrokenPipeError'), 'index.ts guards uncaughtException against EPIPE')
  check(mainIndex.includes('safeSend(window'), 'index.ts has safeSend utility for IPC')
  check(mainIndex.includes('safeSend(overlayWindow'), 'index.ts uses safeSend for overlay events')
  const tts = readFile('src/main/ai/tts.ts')
  check(tts.includes('speakOpenAI'), 'TTS has OpenAI fallback logic')
  check(tts.includes('SpeakResult') && tts.includes('providerUsed'), 'TTS returns provider details')
  check(tts.includes('[TTS] speak called') && tts.includes('[TTS] ElevenLabs failed'), 'TTS logs speak attempts and safe fallback errors')
  check(tts.includes('using macOS fallback') && tts.includes('safeLog'), 'TTS clearly logs when falling back to macOS say')
  check(tts.includes('ELEVENLABS_VOICE_ID') && tts.includes('ELEVENLABS_MODEL_ID'), 'TTS supports custom ElevenLabs voice and model IDs')
  ...
  check(overlayAppBody.includes('[ULTRA] skipped because silent mode'), 'renderer logs silent skips')
  ...
  check(overlayAppBody.includes('ultraState === \'thinking\'') && overlayAppBody.includes('ultraState === \'speaking\''), 'OverlayApp shows ultra state visibility')
  check(overlayAppBody.includes('setTimeout') && overlayAppBody.includes('converse timeout'), 'Ultra has hard timeouts for conversation')
