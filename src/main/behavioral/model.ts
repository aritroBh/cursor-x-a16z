...
const MOODS: SpecMood[] = ['idle', 'thinking', 'stuck', 'flow', 'celebrating', 'mirroring', 'judging']
...
export function normalizeBehavioralFrame(value: any): BehavioralFrame {
  ...
  return {
    ...
    cursorX: typeof raw.cursorX === 'number' && Number.isFinite(raw.cursorX) ? Math.min(100, Math.max(0, raw.cursorX)) : undefined,
    cursorY: typeof raw.cursorY === 'number' && Number.isFinite(raw.cursorY) ? Math.min(100, Math.max(0, raw.cursorY)) : undefined,
    cursorDelta,
    ...
    app: typeof raw.app === 'string' && raw.app.trim() ? raw.app.trim() : undefined,
    targetLabel: typeof raw.targetLabel === 'string' && raw.targetLabel.trim() ? raw.targetLabel.trim() : undefined,
...
export function aggregateBehavioralSignature(
  ...
  const normalized = Array.isArray(frames) ? frames.map(normalizeBehavioralFrame).slice(-160) : []
  ...
  const totals = normalized.reduce(
    (acc, frame, index) => {
      ...
      const deltaDistance = frame.cursorDelta
        ...
        : previous && typeof frame.cursorX === 'number' && typeof frame.cursorY === 'number'
          ? Math.hypot(frame.cursorX - (previous.cursorX ?? frame.cursorX), frame.cursorY - (previous.cursorY ?? frame.cursorY))
          : 0
      const directness = clamp01(deltaDistance / (deltaDistance + frame.dwellMs / 140 + 1), 0.45)
      const actionWeight =
        frame.actionType === 'click' || frame.actionType === 'type' || frame.actionType === 'accept'
          ? 1
          : frame.actionType === 'repeat-click' || frame.actionType === 'override' || frame.actionType === 'correction'
            ? 0.78
      ...
      const impulsivity = clamp01(actionWeight * (1 - dwellNorm) * (0.55 + directness * 0.45), 0)
...
export function personalityForState(state: BehavioralState): BehavioralCheckpoint['specPersonality'] {
...
export function humanCommitMessage(previous: BehavioralState | null | undefined, next: BehavioralState): string {
...
export function createBehavioralCheckpoint(args: {
  ...
  const id = safeLabel(args.id, `behavior-${sessionN}-${timestamp.replace(/[^0-9]/g, '').slice(0, 14)}`)
  ...
  return {
    ...
    label: safeLabel(args.label, `Session ${sessionN}: ${signature.moodLabel} you`),
    commitMessage: safeLabel(args.commitMessage, humanCommitMessage(previousSignature, signature)),
    synthetic: args.synthetic === true
  }
}

export function normalizeBehavioralCheckpoint(value: any, fallbackId?: string): BehavioralCheckpoint {
  const raw = isRecord(value) ? value : {}
  ...
  const checkpoint = createBehavioralCheckpoint({
    ...
    label: safeLabel(raw.label, `Session ${sessionN}: ${signature.moodLabel} you`),
    commitMessage: safeLabel(raw.commitMessage, humanCommitMessage(null, signature)),
    synthetic: raw.synthetic === true
  })
  ...
  return {
    ...
    specPersonality: isRecord(raw.specPersonality)
      ? {
          ...
          eyeShape: ['wide', 'focused', 'sleepy', 'judging', 'glow'].includes(raw.specPersonality.eyeShape)
...
export function diffBehavioralCheckpoints(from: BehavioralCheckpoint, to: BehavioralCheckpoint): BehavioralDiff {
...
export function blendBehavioralStates(a: BehavioralState, b: BehavioralState, t: number): BehavioralState {
