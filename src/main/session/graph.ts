...
export function markNodeComplete(graph: LearningGraph, nodeId: string): LearningGraph {
...
export function getAvailableNodes(graph: LearningGraph): Node[] {
  return Object.values(graph.nodes).filter((node) =>
    node.prerequisites.every((prerequisite) => graph.nodes[prerequisite]?.completed)
  )
}
