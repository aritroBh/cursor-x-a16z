...
function runCheck() {
  ...
  try {
    ...
    if (output) {
      console.error('\x1b[31m[SECURITY FAILURE] Found potential leaked NVIDIA API keys:\x1b[0m');
      console.error(output);
      process.exit(1);
    }
  } catch (error: any) {
    console.warn(`[SECURITY WARNING] Security check script encountered an error: ${error.message}`);
  }
}
