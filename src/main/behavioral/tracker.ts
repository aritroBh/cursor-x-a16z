...
import type { BehavioralCheckpoint, BehavioralFrame, BehavioralState, LearningGraph } from '../session/types'
...
let lastClick: { x?: number; y?: number; t: number; count: number } | null = null
let cleanupListeners: Array<() => void> = []
let stateEmitter: ((state: BehavioralState, frame: BehavioralFrame) => void) | null = null
...
function cursorPercentFromEvent(event: any): { x?: number; y?: number; delta?: { dx: number; dy: number } } {
  const x = typeof event?.x === 'number' && Number.isFinite(event.x) ? Math.min(100, Math.max(0, event.x)) : undefined
  const y = typeof event?.y === 'number' && Number.isFinite(event.y) ? Math.min(100, Math.max(0, event.y)) : undefined
  const delta =
...
async function cursorPercentSafe(event: any): Promise<{ x?: number; y?: number; delta?: { dx: number; dy: number } }> {
  try {
...
function isBackspaceOrDelete(event: any): boolean {
  return event?.keycode === UiohookKey.Backspace || event?.keycode === UiohookKey.Delete
}
...
export function recordBehavioralReward(input: any): BehavioralFrame {
  const reward = typeof input?.reward === 'number' && Number.isFinite(input.reward) ? Math.min(1, Math.max(0, input.reward)) : 0.5
  return pushFrame({
    t: Date.now(),
    dwellMs: 350,
    actionType: reward >= 0.7 ? 'accept' : reward <= 0.25 ? 'override' : 'hesitation',
    revisionSignal: reward <= 0.25 ? 0.8 : 0.05,
    targetLabel: typeof input?.targetLabel === 'string' ? input.targetLabel : 'reward signal'
  })
}
...
export function rewardFromFeedback(input: any): number {
  ...
  const correctionCount =
    typeof input?.correctionCount === 'number' && Number.isFinite(input.correctionCount)
...
export function recordBehavioralFeedback(input: any): { frame: BehavioralFrame; reward: number } {
  ...
  const frame = pushFrame({
    t: Date.now(),
    dwellMs:
      typeof input?.hesitationMs === 'number' && Number.isFinite(input.hesitationMs)
...
export function recordReplayBehavioralEvent(kind: 'retry' | 'failure', targetLabel?: string): BehavioralFrame {
  return pushFrame({
    t: Date.now(),
    dwellMs: Math.max(0, Date.now() - lastActionAt),
...
export function seedDemoCheckpoints(graph: LearningGraph): {
  ...
  const demoStates: Array<{ label: string; commitMessage: string; signature: BehavioralState }> = [
    {
